...
class TestLoadResult:
    ...
    @pytest.mark.parametrize(
        "record_type,record_id,status,message,expected",
        [
            (
                "M3U_CHANNEL",
                "ch1",
                "inserted",
                "Success",
                "M3U_CHANNEL[ch1]: inserted - Success",
            ),
            ...
            (
                ...
                "error",
                "Database error",
                "M3U_CHANNEL[ch3]: error - Database error",
            ),
            (
                "M3U_CHANNEL",
                "ch4",
                "skipped",
                "Already exists",
                "M3U_CHANNEL[ch4]: skipped - Already exists",
            ),
        ],
    )
    def test_load_result_string_representation(
        self, record_type, record_id, status, message, expected
    ):
