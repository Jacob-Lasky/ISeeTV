            ...
            if validation_errors:
                ...
                if self.scheduler:
                    try:
                        ...
                        results["errors"].append(f"Failed to schedule {source.name}: {e}")
...
async def ingest_callback_wrapper(source_name: str, file_type: str) -> None:
    ...
    try:
        ...
        if not file_metadata or not file_metadata.local_path:
            raise ValueError(f"No {file_type.upper()} file defined for source '{source_name}'")
...
def initialize_scheduler(sources_file: str) -> None:
    """
    Initialize the global scheduler with default settings.
    
    Args:
        sources_file: Path to sources configuration file
    """
    log_function("Initializing global scheduler")
    scheduler_manager.initialize(
        download_callback=download_callback_wrapper,
        ingest_callback=ingest_callback_wrapper,
        sources_file=sources_file
    )
