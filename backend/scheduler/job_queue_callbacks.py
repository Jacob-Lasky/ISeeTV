...
async def refresh_job_callback_wrapper(source_name: str, file_type: Literal["m3u", "epg"]) -> None:
    """
    ...
    try:
        ...
        log_function(f"Scheduler refresh job queued for {source_name} {file_type}: {job_id}")
...
async def _execute_refresh_job(
    ...
    try:
        ...
        if not os.path.exists(file_path):
            raise ValueError(f"Downloaded file '{file_path}' not found for source '{source_name}'")
        ...
        log_function(f"Refresh job completed successfully for {source_name} {file_type}")
