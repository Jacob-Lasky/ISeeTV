...
class RefreshScheduler:
    ...
    def schedule_source_refresh(self, source: Source) -> None:
        ...
        if job_ids:
            ...
            logger.info(f"Scheduled {len(job_ids)} refresh jobs for source {source.name}")
    
    def _schedule_file_refresh(self, source: Source, file_type: Literal["m3u", "epg"]) -> Optional[str]:
        """
        ...
        try:
            ...
            logger.info(f"Scheduled {file_type} refresh for {source.name} at {next_refresh}")
            return job_id
    ...
    def _calculate_next_refresh(self, source: Source) -> Optional[datetime]:
        ...
        try:
            ...
            tz = ZoneInfo(source.source_timezone) if source.source_timezone else ZoneInfo("UTC")
    ...
    def _create_cron_trigger(self, source: Source):
        ...
        if source.refresh_every_hours >= 24:
            return IntervalTrigger(
                hours=source.refresh_every_hours,
                timezone=tz
            )
    ...
    async def _refresh_file_job(self, source_name: str, file_type: Literal["m3u", "epg"]) -> None:
        """
