...
def get_streams_query(
    ...
    """
    log_function(f"Executing streams query: page={page}, size={page_size}, source={source}, group={group}")
    ...
    try:
            ...
            for column, value in column_filters.items():
                if value:
                    if column == "name":
                        ...
                        where_conditions.append("e.display_name LIKE :display_name_filter")
        ...
        # Add ORDER BY
        valid_sort_fields = ["name", "tvg_id", "display_name", "group", "source", "program_count", "created_at", "updated_at"]
        ...
        for row in result:
            stream = StreamChannel(
                ...
                next_program_start=datetime.fromisoformat(row.next_program_start.replace('Z', '+00:00')) if row.next_program_start and isinstance(row.next_program_start, str) else row.next_program_start
            )
            streams.append(stream)
            
        logger.info(f"Retrieved {len(streams)} streams out of {total_count} total")
...
def get_stream_programs_query(
    ...
    """
    log_function(f"Executing stream programs query: channel_id={channel_id}, source={source}")
    ...
    try:
            ...
            for column, value in column_filters.items():
                if value:
                    if column == "title":
                        ...
                        where_conditions.append("p.description LIKE :description_filter")
        ...
        # Add ORDER BY
        valid_sort_fields = ["start_time", "end_time", "title", "created_at", "updated_at"]
        if sort_field not in valid_sort_fields:
        ...
        for row in result:
            program = StreamProgram(
                ...
                start_time=datetime.fromisoformat(row.start_time.replace('Z', '+00:00')) if isinstance(row.start_time, str) else row.start_time,
                end_time=datetime.fromisoformat(row.end_time.replace('Z', '+00:00')) if isinstance(row.end_time, str) else row.end_time,
                title=row.title,
        ...
        logger.info(f"Retrieved {len(programs)} programs out of {total_count} total for channel {channel_id}")
...
def precompute_streams_filter_values(session: Session) -> None:
    ...
    try:
        ...
        if all_values:
            ...
            session.commit()
            logger.info(f"Added {len(all_values)} filter values for streams view")
        else:
