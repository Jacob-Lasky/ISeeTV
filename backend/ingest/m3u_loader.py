...
Atomic, modular functions for loading M3U channels into database.
"""
...
def _upsert_m3u_channel(session: Session, channel: M3uChannel) -> LoadResult:
    """Atomic function to upsert a single M3U channel"""
    try:
...
async def load_m3u_channels_async(
    ...
    logger.info(f"Starting async M3U channel load from {file_path} for source {source_name}")
